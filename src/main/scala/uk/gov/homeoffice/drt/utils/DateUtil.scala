...
object DateUtil {
  ...
  val `yyyy-MM-ddTHH:mm:ss.SSSZ` : String => Date = date => new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").parse(date)
  ...
  val `yyyy-MM-ddTHH:mm:ss.SSSZ_LocalDateFormatter` = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
